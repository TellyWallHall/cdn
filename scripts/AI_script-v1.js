        ...
        async function askAi(systemPrompt, message, maxTokenOutput=10, debug=false) {
            ...
            if(debug) {
                alert(`System prompt: ${systemPrompt}`, `Message: ${message}`, `Max output tokens: ${maxTokenOutput}`);
            }
            ...
            try {
                if(debug) {
                    alert(`Calling cloudflare`);
                }
                const response = await callCloudflareAPI(false, true);
                return response;
            } catch (error) {
