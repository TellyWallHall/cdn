        ...
        async function cleanupOldGames() {
            ...
            // Clean up waiting games older than 5 minutes
            const waitingSnapshot = await database.ref('chess/waiting').once('value');
            const waitingGames = waitingSnapshot.val();
            if (waitingGames) {
                for (const gameId in waitingGames) {
                    const game = waitingGames[gameId];
                    if (game.createdAt < now - (5 * 60 * 1000)) {
                        await database.ref(`chess/waiting/${gameId}`).remove();
                    }
                }
            }

            // Clean up abandoned active games (no moves in 1 hour)
            const gamesSnapshot = await database.ref('chess/games').once('value');
            const games = gamesSnapshot.val();
            if (games) {
                for (const gameId in games) {
                    const game = games[gameId];
                    if (game.status === 'active' && game.startTime < oneHourAgo) {
                        await database.ref(`chess/games/${gameId}`).remove();
                    }
                    // Clean up finished games older than 1 day
                    if (game.winner && game.endTime && game.endTime < oneDayAgo) {
                        await database.ref(`chess/games/${gameId}`).remove();
                    }
                }
            }
            ...
            const challengesSnapshot = await database.ref('chess/challenges').once('value');
            const challenges = challengesSnapshot.val();
            if (challenges) {
                for (const challengeId in challenges) {
                    const challenge = challenges[challengeId];
                    if (challenge.timestamp < now - (5 * 60 * 1000)) {
                        await database.ref(`chess/challenges/${challengeId}`).remove();
                    }
                }
            }

            // Clean up old chat messages (keep only last 100)
            const chatSnapshot = await database.ref('chess/chat').orderByChild('timestamp').once('value');
            const messages = [];
            ...
            if (messages.length > 100) {
                ...
                for (const msg of toDelete) {
                    await database.ref(`chess/chat/${msg.key}`).remove();
                }
            }
        }
        ...
        function listenForChallenges() {
            ...
            if (gameState.challengeListener) {
                database.ref('chess/challenges').off('child_added', gameState.challengeListener);
            }

            gameState.challengeListener = async (snapshot) => {
                const challenge = snapshot.val();
            ...
            database.ref('chess/challenges').on('child_added', gameState.challengeListener);
        }
            ...
            players.forEach((player, index) => {
                ...
                item.innerHTML = `
                    ...
                    <div class="friend-info">
                        <div class="friend-avatar">${player.username[0].toUpperCase()}</div>
                        <div class="friend-details">
                            <div class="friend-name">${player.username}${isCurrentUser ? ' (You)' : ''}${onlineStatus}</div>
        ...
        document.getElementById('premovesToggle').addEventListener('click', function() {
            this.classList.toggle('active');
            settings.premoves = this.classList.contains('active');
        ...
        document.getElementById('turnTimeLimitSlider').addEventListener('input', function() {
            settings.turnTimeLimit = parseInt(this.value);
            document.getElementById('turnTimeLimitValue').textContent = this.value;
        });
        ...
        function listenToGame() {
            ...
            // Clean up old draw offer listener if exists
            if (gameState.drawOfferListener) {
                database.ref(`chess/games/${gameState.gameId}/drawOffer`).off('value', gameState.drawOfferListener);
            }
            ...
            gameRef.on('value', (snapshot) => {
                ...
                // Check for draw offers - only if in this specific game
                if (game.drawOffer && game.drawOffer.from !== currentUser.id && 
                    (game.whitePlayer.id === currentUser.id || game.blackPlayer.id === currentUser.id)) {
                    const opponentName = game.drawOffer.from === game.whitePlayer.id ? 
                        game.whitePlayer.username : game.blackPlayer.username;
                    
                    // Only show if modal isn't already showing
                    if (!document.getElementById('drawOfferModal').classList.contains('show')) {
                ...
                // Execute premove if it's now our turn
                if (gameState.premove && gameState.currentTurn === gameState.playerColor && !gameState.gameOver) {
                    const { from, to } = gameState.premove;
                    
                    // Validate premove is still legal
                    const piece = gameState.board[from.row][from.col];
                    if (piece && isPlayerPiece(piece)) {
                        const validMoves = getValidMoves(from.row, from.col);
                        const isStillValid = validMoves.some(m => m.row === to.row && m.col === to.col);
                        
                        if (isStillValid) {
                            gameState.selectedSquare = from;
                            gameState.validMoves = validMoves;
                            
                            // Execute the premove after a short delay for visual feedback
                            setTimeout(() => {
                                makeMove(from, to);
                            }, 100);
                        }
        ...
        function updateGameUI() {
            ...
            for (let row = 0; row < 8; row++) {
                for (let col = 0; col < 8; col++) {
                    ...
                    if (gameState.premove && 
                        ((gameState.premove.from.row === displayRow && gameState.premove.from.col === displayCol) ||
                         (gameState.premove.to.row === displayRow && gameState.premove.to.col === displayCol))) {
            ...
            if (gameState.turnTimeoutId) {
                clearTimeout(gameState.turnTimeoutId);
            }

            const turnLimitMs = settings.turnTimeLimit * 60 * 1000;

            gameState.moveTimerInterval = setInterval(() => {
                const elapsed = Math.floor((Date.now() - gameState.turnStartTime) / 1000);
                const remaining = Math.max(0, settings.turnTimeLimit * 60 - elapsed);
                const minutes = Math.floor(remaining / 60);
                const seconds = remaining % 60;
                const timeStr = `${minutes}:${seconds.toString().padStart(2, '0')}`;
                
                const activeTimer = isPlayerTurn ? document.getElementById('playerTimer') : document.getElementById('opponentTimer');
                const inactiveTimer = isPlayerTurn ? document.getElementById('opponentTimer') : document.getElementById('playerTimer');
                
                activeTimer.textContent = timeStr;
                inactiveTimer.textContent = '--:--';
                
                // Remove all timer classes
                activeTimer.className = 'timer';
                inactiveTimer.className = 'timer';
                
                // Add appropriate class based on remaining time
                if (remaining < 10) {
                    activeTimer.classList.add('critical');
                } else if (remaining < 30) {
                    activeTimer.classList.add('warning');
                }

                // Vibrate on last 10 seconds
                if (remaining <= 10 && remaining > 0 && settings.vibration && navigator.vibrate && isPlayerTurn) {
                    navigator.vibrate(100);
                }

                if (remaining === 0 && !gameState.gameOver) {
                    clearInterval(gameState.moveTimerInterval);
                    if (!gameState.isLocalGame && isPlayerTurn) {
                        endGame(gameState.playerColor === 'white' ? 'black' : 'white', 'time forfeit');
                    } else if (!gameState.isLocalGame && !isPlayerTurn) {
                        endGame(gameState.playerColor, 'opponent time forfeit');
                    }
                }
            }, 1000);

            if (!gameState.isLocalGame && isPlayerTurn) {
                gameState.turnTimeoutId = setTimeout(async () => {
                    if (!gameState.gameOver && gameState.currentTurn === gameState.playerColor) {
                        await endGame(gameState.playerColor === 'white' ? 'black' : 'white', 'time forfeit');
                    }
                }, turnLimitMs);
            }
        ...
        function handleSquareClick(row, col) {
            ...
            // If it's our turn, handle normally
            ...
            } else if (settings.premoves) {
                // Not our turn - handle premove (only if enabled)
                const piece = gameState.board[row][col];

                if (gameState.selectedSquare) {
                    // Check if this would be a valid move when it becomes our turn
                    const selectedPiece = gameState.board[gameState.selectedSquare.row][gameState.selectedSquare.col];
                    if (selectedPiece && isPlayerPiece(selectedPiece)) {
                        // Set premove
                        gameState.premove = {
                            from: gameState.selectedSquare,
                            to: { row, col }
                        };
                        showToast('Premove set');
                        gameState.selectedSquare = null;
                        gameState.validMoves = [];
                        updateGameUI();
                    }
        ...
        document.getElementById('backToMenuBtn').addEventListener('click', () => {
            ...
            if (gameState.gameId) {
                database.ref(`chess/games/${gameState.gameId}`).off();
            }
