...
updateMediaSession(item) {
    ...
    navigator.mediaSession.setActionHandler("previoustrack", () => {
        const prev = this.getPreviousTrack();
        if (prev) this.playMedia(prev);
    });
    ...
    navigator.mediaSession.setActionHandler("seekto", (details) => {
        const isVideo = ["Movie", "Episode", "Series"].includes(APP.currentMedia.Type);
  ...
  try {
    await fetch(`${server.url}/Sessions/Playing`, {
      ...
      body: JSON.stringify({
        ...
        PlaySessionId: this.getDeviceId()
      })
    });
  } catch (e) {
...
getStreamUrl(item) {
    ...
    // Let Jellyfin transcode incompatible codecs to h264/aac
    return `${server.url}/Videos/${item.Id}/master.m3u8?api_key=${server.token}&MediaSourceId=${item.Id}&VideoCodec=h264&AudioCodec=aac&MaxStreamingBitrate=${maxBitrate}&TranscodingMaxAudioChannels=2&PlaySessionId=${this.getDeviceId()}`;
},
...
attachVideo(videoElement, url) {
    ...
    if (url.includes('.m3u8') && Hls.isSupported()) {
        ...
        hls.on(Hls.Events.ERROR, (event, data) => {
            if (data.fatal) {
                ...
                if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
                    ...
                    this.showToast("Playback error — try a different quality", "error");
                }
            }
        });
        APP.hlsInstance = hls;
        ...
        // Fallback to direct URL (non-HLS)
        videoElement.src = url;
        videoElement.play();
    }
},
