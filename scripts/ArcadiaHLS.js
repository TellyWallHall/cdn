      ...
      const APP = {
        ...
        currentPlaySessionId: null,        hlsInstance: null
      };
...
getStreamUrl(item) {
    ...
    return `${server.url}/Videos/${item.Id}/master.m3u8?api_key=${server.token}&MediaSourceId=${item.Id}&VideoCodec=h264&AudioCodec=aac&MaxStreamingBitrate=${maxBitrate}&TranscodingMaxAudioChannels=2&PlaySessionId=${APP.currentPlaySessionId}`;
},
...
attachVideo(videoElement, url) {
    ...
    // Fully reset the video element before loading new source
    videoElement.pause();
        ...
        hls.on(Hls.Events.ERROR, (event, data) => {
            if (data.fatal) {
                ...
                if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
                    ...
                    this.showToast("HLS failed — trying direct stream", "info");
                    const fallbackUrl = url.replace('/master.m3u8', '/stream')
                        .replace(/&VideoCodec=[^&]*/, '')
                        .replace(/&AudioCodec=[^&]*/, '')
                        .replace(/&TranscodingMaxAudioChannels=[^&]*/, '');
...
hlsInstance = null;
    ...
    // Use HLS-aware playbackconsole.log(API.getStreamUrl(item));
